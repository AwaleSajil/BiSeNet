...
#Create a varibale(global) to hold instance of SummaryWriter for later
writter = None
...
def parse_args():
    ...
    parse.add_argument('-se', '--saveOnEveryEpoch', type=int, help = "Save a checkpoint after this many epoch", default=1)
    ...
    parse.add_argument('-tb', '--tensorBoardLogDir', type=str, help="folder to which logs are to written for tensorboard" ,default=None)
...
def addGraph_Tensorboard(net, dl):
    global writter
    try:
        images, _ = next(iter(dl))
        images = images.cuda()
        writter.add_graph(net, images)
    except:
        images = torch.randn(8, 3, 1024, 1024).cuda()
        writter.add_graph(net, images)
    writter.close()

def addImage_Tensorboard(dl):
    global writter
    examples = iter(dl)
    example_img, example_label = examples.next()
...
def setupTensorBoardWriter():
    global writter
    if args.tensorBoardLogDir == None:
        return
    tz = pytz.timezone('Asia/Kathmandu')
    now = datetime.datetime.now(tz=tz)
    path = str(args.tensorBoardLogDir) + str(now)
    writter = SummaryWriter(path)

def main():
    if not osp.exists(cfg.respth): os.makedirs(cfg.respth)
    setup_logger('{}-train'.format(cfg.model_type), cfg.respth)
    print("Args sent: ", args.saveCheckpointDir, args.loadCheckpointLocation, args.saveOnEveryEpoch, args.tensorBoardLogDir)
